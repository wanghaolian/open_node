        ...
        .then(res => {
            if (res.length > 0) {
                ...
                if (res[0].reviewStatus == 4) {
                    data.orderStatus = 2;
                    orderMemo = "识别异常";
                    let orderConfirmSQL = `select err_type_content AS errContent from order_confirm_record where act_type = 4 and order_id = ` +res[0].orderId +` ORDER BY id desc`;
                    sequelize.query(orderConfirmSQL, { type: sequelize.QueryTypes.SELECT })
                    .then(resOrderConfirm => {
                        if (resOrderConfirm.length > 0){
                            data.orderMemo = orderMemo + ":"+resOrderConfirm[0].errContent;
                        }
                        callback(data);
                    });
                }
